        ...
        if (User.objects.filter(username=voterid)).exists():
            ...
            if Voters.objects.filter(voterid_no=voterid):
                ...
                url = "http://2factor.in/API/V1/" + \
                    settings.TWO_FACTOR_API_KEY + "/SMS/" + user_phone + "/AUTOGEN"
...
def otp(request):
    if (request.method == "POST"):
        ...
        url = "http://2factor.in/API/V1/" + settings.TWO_FACTOR_API_KEY + \
            "/SMS/VERIFY/" + \
...
def register(request):
    if (request.method == 'POST'):
        ...
        #             cv2.imwrite(str(BASE_DIR) + "/TrainingImage/" + name + "." +
        ...
        #                       for f in os.listdir(path) if f != ".gitkeep"]
        #         faces = []
        #         Ids = []
...
def velection(request):
    ...
    if Election.objects.filter(state=vdetail.state, status=status):
        ...
        election_json = velection.__dict__['e']
        print('election_Type', election_json.election_type)
        print('election_Type1', velection.e.election_type)

        print('election', election_json)
    ...
    if request.method == "POST":
        ...
        if vote.v.has_voted == 'no':
            ...
            if flag == 1:
                ...
                url = "http://2factor.in/API/V1/" + \
                    settings.TWO_FACTOR_API_KEY + "/SMS/" + vmobno + "/AUTOGEN"
                response = requests.request("GET", url)
                data = response.json()
                request.session['otp_session_data'] = data['Details']
                response_data = {'Message': 'Success'}
