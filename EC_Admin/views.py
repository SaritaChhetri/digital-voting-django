...
def modifyelection(request):
    if Election.objects.filter(status='active'):
        ...
        election_json = serializers.serialize('json', e)
...
def modify_election(request):
    if request.method == "POST":
        ...
        modifyele.end_date = formatted_end_date

    # Parse the time string into a time object
        time_str = end_time.replace('.', '')
        time_obj = datetime.strptime(time_str, "%I:%M %p")

        # Format the time object into HH:MM:SS format
        formatted_time = time_obj.strftime("%H:%M:%S")
        modifyele.end_time = formatted_time
        modifyele.save()
